...
{
    public delegate int LearnedFunc(int[] a);
...
3 - exit";
            ...
            while (option != 3)
            {
                ...
                {
                    if(e.Message == "No program was found for this specification.")
                    {
                        Console.Error.WriteLine(e.Message);//+ e.StackTrace);
                        Environment.Exit(1);
                    }
                    Console.Error.WriteLine("Exception Found: "+e.Message);
                }
            }
        }
        ...
        {
            Console.Out.Write("Provide a new input-output example (e.g., \"1 2\",\"3\"): ");
            try
            {
                ...
                // Console.WriteLine("{0} & {1}", inputExample, outputExample);
                // Console.WriteLine(inputExample.GetType());
            ...
            Console.WriteLine("Learning Grammar");
            var scoreFeature = new RankingScore(grammar);
            ProgramSet topPrograms = prose.LearnGrammarTopK(spec, scoreFeature, 10, null);
            //ProgramSet learned = prose.LearnGrammar(spec);
            Console.WriteLine("GRAMMAR LEARNED");
            //if (learned?.Size <= 0)
            ...
            Console.Out.WriteLine("Top learned programs:");
            var counter = 1;
